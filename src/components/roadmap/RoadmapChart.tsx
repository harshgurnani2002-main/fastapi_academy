...
const colorMap: Record<string, { border: string, bg: string, text: string, glow: string }> = {
  gray: { border: 'border-slate-500', bg: 'bg-slate-500', text: 'text-slate-600', glow: 'shadow-[0_0_15px_rgba(100,116,139,0.5)]' },
  blue: { border: 'border-blue-500', bg: 'bg-blue-500', text: 'text-blue-600', glow: 'shadow-[0_0_15px_rgba(59,130,246,0.5)]' },
  red: { border: 'border-red-500', bg: 'bg-red-500', text: 'text-red-600', glow: 'shadow-[0_0_15px_rgba(239,68,68,0.5)]' },
  orange: { border: 'border-orange-500', bg: 'bg-orange-500', text: 'text-orange-600', glow: 'shadow-[0_0_15px_rgba(249,115,22,0.5)]' },
  green: { border: 'border-green-500', bg: 'bg-green-500', text: 'text-green-600', glow: 'shadow-[0_0_15px_rgba(34,197,94,0.5)]' },
  yellow: { border: 'border-yellow-500', bg: 'bg-yellow-500', text: 'text-yellow-600', glow: 'shadow-[0_0_15px_rgba(234,179,8,0.5)]' },
  purple: { border: 'border-purple-500', bg: 'bg-purple-500', text: 'text-purple-600', glow: 'shadow-[0_0_15px_rgba(168,85,247,0.5)]' },
};
...
export function RoadmapChart() {
  return (
    <div className="relative w-full max-w-4xl mx-auto py-12">
      ...
      <div className="absolute left-6 md:left-1/2 top-0 bottom-0 w-1 bg-slate-200 -translate-x-1/2 rounded-full overflow-hidden">
        ...
        {roadmapData.map((node, index) => {
          ...
          return (
            <div key={node.id} className={`relative flex flex-col md:flex-row items-center ${isEven ? 'md:flex-row-reverse' : ''}`}>
              ...
              {/* Connector Point */}
              <div className={`absolute left-6 md:left-1/2 -translate-x-1/2 w-6 h-6 rounded-full border-4 border-white z-10 ${colors.bg} ${isCurrent ? colors.glow + ' animate-pulse' : ''} ${isCompleted ? '' : 'opacity-50'}`}>
                {isCompleted && (
              ...
              <div className={`w-full md:w-1/2 pl-16 md:pl-0 ${isEven ? 'md:pr-12' : 'md:pl-12'} flex flex-col ${isEven ? 'md:items-end md:text-right' : 'md:items-start md:text-left'}`}>
                <Link href={node.link} className="block w-full group">
                  <div className={`p-6 rounded-2xl bg-white border ${isCurrent ? colors.border : 'border-slate-200'} transition-all duration-300 hover:border-slate-400 hover:bg-slate-50 hover:shadow-xl shadow-sm`}>
                    ...
                    {isCurrent && (
                      <div className="mt-4 inline-flex items-center gap-2 px-3 py-1 rounded-full bg-orange-100 text-orange-700 text-xs font-semibold border border-orange-200">
      ...
      <div className="relative mt-24 flex flex-col items-center">
        <div className="w-12 h-12 rounded-full border-4 border-white bg-gradient-to-tr from-orange-500 to-yellow-500 shadow-[0_0_30px_rgba(249,115,22,0.4)] z-10 flex items-center justify-center">
          <svg className="w-6 h-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" />
          </svg>
        </div>
        <div className="mt-6 text-center">
          <h2 className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-yellow-500 mb-2">
